...
const Checkout = () => {
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-[#FFF8F0] flex items-center justify-center px-4 pb-24 md:pb-8">
        <div className="bg-white rounded-3xl shadow-lg p-8 text-center max-w-sm w-full">
          ...
          <h2 className="text-xl font-extrabold text-stone-800 mb-2">سلتك فارغة</h2>
          <p className="text-sm text-stone-500 mb-6">
            أضف أطباقاً للسلة قبل إكمال الطلب
          </p>
          <Link
            to="/cooks"
            className="inline-flex items-center gap-2 bg-gradient-to-l from-orange-500 to-orange-600 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-orange-500/30 active:scale-95 transition"
          >
            تصفّح الطباخات
          </Link>
        </div>
  ...
  const steps = [
    ...
    { num: 2, label: 'التوصيل', icon: Calendar },
        ...
        {currentStep === 1 && (
          <section className="animate-slide-up space-y-4">
            <SectionTitle
              ...
              subtitle="لكي تتواصل معك الطباخة بسهولة"
            />
            ...
            <div className="bg-white rounded-3xl shadow-sm p-5 space-y-4">
              ...
              <InputField
                ...
                dir="ltr"
                hint="10 أرقام، يبدأ بـ 0"
                isValid={customerPhone.length === 10 ? validatePhone(customerPhone) : null}
              />
        ...
        {currentStep === 2 && (
          <section className="animate-slide-up space-y-4">
            <SectionTitle
              icon={Calendar}
              title="موعد الاستلام"
              subtitle="متى تريد الحصول على طلبك؟"
            />

            <div className="bg-white rounded-3xl shadow-sm p-5 space-y-4">
              {/* نوع الطلب - 2 بطاقات كبيرة */}
              <div className="grid grid-cols-2 gap-3">
                <OrderTypeCard
                  selected={orderType === 'instant'}
                  onClick={() => setOrderType('instant')}
                  icon={Zap}
                  title="فوري"
                  subtitle="في أقرب وقت"
                  color="orange"
                />
                <OrderTypeCard
                  selected={orderType === 'scheduled'}
                  onClick={() => setOrderType('scheduled')}
                  icon={Clock}
                  title="مسبق"
                  subtitle="في موعد تختاره"
                  color="blue"
                />
              </div>

              {/* تاريخ ووقت (للمسبق فقط) */}
              {orderType === 'scheduled' && (
              ...
              <div className="pt-2 border-t border-stone-100">
                ...
                <textarea
                  ...
                  placeholder="مثلاً: بدون فلفل حار، توصيل قبل الساعة..."
                />
                <p className="text-[11px] text-stone-500 mt-1">
                  {notes.length}/200 حرف
                </p>
              </div>
            </div>
          </section>
        ...
        {currentStep === 3 && (
          <section className="animate-slide-up space-y-4">
            <SectionTitle
              icon={Check}
              title="راجع طلبك"
              subtitle="تأكد من التفاصيل قبل التأكيد النهائي"
            />
            ...
            <div className="bg-white rounded-3xl shadow-sm overflow-hidden">
              <div className="bg-gradient-to-l from-orange-50 to-white px-4 py-3 border-b border-orange-100 flex items-center gap-2">
                ...
                <h3 className="text-sm font-extrabold text-stone-800">
                  أطباقك ({cookGroups.length} {cookGroups.length === 1 ? 'طباخة' : 'طباخات'})
                </h3>
              </div>
                ...
                {cookGroups.map((group) => (
                  <div key={group.cookId} className="p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="font-extrabold text-orange-600 text-xs">
                        {group.cookName}
                      </p>
                    </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm overflow-hidden">
              <div className="bg-gradient-to-l from-stone-50 to-white px-4 py-3 border-b border-stone-100 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] font-bold text-orange-600 hover:text-orange-700 active:scale-95 transition"
                >
                  تعديل
                </button>
              </div>
              <div className="p-4 space-y-2.5">
                <InfoRow icon={User} label="الاسم" value={customerName} />
                <InfoRow icon={Phone} label="الهاتف" value={customerPhone} ltr />
                {customerAddress && (
                  <InfoRow icon={MapPin} label="العنوان" value={customerAddress} />
                )}
                <InfoRow
                  icon={orderType === 'instant' ? Zap : Clock}
                  label="نوع الطلب"
                  value={
                    orderType === 'instant'
                      ? 'فوري'
                      : `مسبق: ${scheduledDate} - ${scheduledTime}`
                  }
                />
                {notes && (
                  <InfoRow icon={MessageSquare} label="ملاحظات" value={notes} />
                )}
              </div>
            </div>
            ...
            {cookGroups.length > 1 && (
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3.5 flex items-start gap-3">
                ...
                <p className="text-xs text-amber-800 font-semibold leading-relaxed">
                  سيتم إرسال {cookGroups.length} طلبات منفصلة (طلب لكل طباخة) وستتواصل معك كل طباخة على حدة عبر واتساب.
                </p>
              </div>
            )}
          </section>
          ...
          <div className="bg-white rounded-3xl shadow-2xl shadow-stone-900/10 border border-stone-100 p-3">
            ...
            <div className="flex items-center justify-between px-2 pb-3">
              <div>
                ...
                <p className="text-xl font-black text-stone-800 leading-tight mt-1">
                  {totalPrice.toLocaleString('ar-DZ')}
                  <span className="text-xs font-bold text-stone-500 mr-1">دج</span>
                </p>
              </div>
              <div className="flex items-center gap-1.5 bg-orange-50 px-2.5 py-1.5 rounded-xl">
                ...
                <span className="text-[11px] font-black text-orange-700">
                  {cookGroups.length} {cookGroups.length === 1 ? 'طلب' : 'طلبات'}
                </span>
              </div>
            </div>
...
function InputField({
  ...
  return (
    <div>
      ...
      {hint && (
        <p className="text-[11px] text-stone-500 mt-1.5 mr-1">{hint}</p>
      )}
    </div>
  );
}
...
function OrderTypeCard({ selected, onClick, icon: Icon, title, subtitle, color }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative p-4 rounded-2xl text-right transition-all active:scale-95 overflow-hidden ${
        selected
          ? `bg-gradient-to-br ${c.bg} text-white shadow-lg ${c.shadow}`
          : 'bg-stone-50 text-stone-700 border-2 border-stone-200 hover:border-stone-300'
      }`}
    >
      {selected && (
      ...
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center mb-2 ${
          selected ? c.iconBg : 'bg-white'
        }`}
      >
        <Icon
          className={`w-5 h-5 ${selected ? 'text-white' : 'text-stone-600'}`}
          strokeWidth={2.3}
        />
      </div>
  ...
  return (
    <div className="flex items-start gap-2 text-sm">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p
          className="text-stone-800 font-bold text-xs mt-1 break-words"
          dir={ltr ? 'ltr' : 'rtl'}
